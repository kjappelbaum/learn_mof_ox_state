...
def evaluate_model(modelpath, featurespath, labelspath, namespath, scalerpath):  # pylint:disable=too-many-locals
    ...
    diffs = [(names[point], i, j) for point, i, j in list(zip(points, predicted, y)) if i != j]
...
def main():  # pylint:disable=too-many-locals
    ...
    for model in models:
        try:
            ...
            scalerpath = glob(os.path.join(folder, '{}-*scaler*.joblib'.format(date)))[0]
            result_list.append(evaluate_model(model, featurespath, labelspath, namespath, scalerpath))
        except Exception:
            pass
