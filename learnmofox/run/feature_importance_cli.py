...
def permuation_importance_wrapper(
    datatuple, model, rounds, metric=balanced_accuracy_score
):
    X = datatuple[0]
    y = datatuple[1]
    name = datatuple[2]
...
def main(  # pylint:disable=too-many-arguments, too-many-locals
    ...
    permuation_importance_partial = partial(
        permuation_importance_wrapper, rounds=rounds, model=model
    )
    ...
    if not use_shap:
        ...
        with open(
            os.path.join(outpath, "permutation_feature_importance.pkl"), "wb"
        ) as fh:
        ...
        for i, shap_value in enumerate(shap_values):
            ...
            expected_value = (
                ...
                if explainer.expected_value[i] is not None
                else None
            )
