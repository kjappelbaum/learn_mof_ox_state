...
def main(
    modelpath, xtrainpath, ytrainpath, xtestpath, ytestpath, outdir
    ...
    relevant_models = [
        [model, joblib.load(model)] for model in models if not "scaler" in model
    ]

    bvpartial = partial(
        bv_decomp_wrapper, xtrain=X_train, ytrain=y_train, xtest=X_test, ytest=y_test
    )
