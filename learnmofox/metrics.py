...
def _bootstrap_metric_fold(
    _, model, X, y, scoring_funcs, sample_idx, rng
...
def bootstrapped_metrics(  # pylint:disable=too-many-arguments
    model, X, y, scoring_funcs, n_rounds=200, seed=1234, max_workers=6
) -> list:
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        for metric in tqdm(
            executor.map(bootstrap_fold_partial, list(rounds)), total=len(rounds)
        ):
...
def _permutation_score_base(
    _, model, X, y, cv, metric_func, shuffle=True
    ...
    for train, test in cv.split(X, y):
        ...
        model_._calibrate_base_estimators(
            "isotonic", X_valid, y_valid
...
def permutation_test(
    model, X, y, rounds=30, metric_func=balanced_accuracy_score, max_workers=12
):  # pylint:disable=too-many-arguments,unused-argument
    cv = StratifiedKFold(10)
    permuted_scores = []
    model_ = model
    score = _permutation_score_base(None, model, X, y, cv, metric_func, shuffle=False)
    base_permutation_score = partial(
        _permutation_score_base, model=model_, X=X, y=y, cv=cv, metric_func=metric_func
    )
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=1) as executor:
        for metric in tqdm(
            executor.map(base_permutation_score, list(rounds)), total=len(rounds)
        ):
            print(metric)
